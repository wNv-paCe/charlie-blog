...
from charlie_blog.email_utils import send_password_reset_email
from charlie_blog.image_utils import (
...
async def update_user(
    ...
    if user_id != current_user.id:
        raise HTTPException(
            ...
            detail="No authorized to update this user",
        )
...
async def delete_user(
    ...
    if user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="No authorized to delete this user",
        )
...
async def upload_profile_picture(
    ...
    if current_user.id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="No authorized to update this user's picture",
        )
